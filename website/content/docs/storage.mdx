...
- **Library → Export** writes your whole library — analyzed repos, the Connections graph, and the scan cache — to one portable JSON file. **Import** restores it (merge or replace). Backups are validated and size-bounded on import, so a bad file fails safe instead of corrupting your library.
