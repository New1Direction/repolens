...
export function mountCanvas(host, inputScene, { onChange } = {}) {
  ...
  const fire = () => {
    if (onChange) onChange(structuredClone(scene));
  };
  const persist = () => {
    if (!onChange) return;
    if (saveTimer) {
  ...
  for (const n of scene.nodes) {
    ...
    title.textContent = n.label; // full label on hover (survives truncation)
  ...
  svg.addEventListener('pointermove', (ev) => {
    if (panning) {
      cam.x = pcx + (ev.clientX - px);
      cam.y = pcy + (ev.clientY - py);
      applyCamera();
    }
  });
  ...
  function moveNode(id, x, y) {
    ...
    for (const e of scene.edges)
      if (e.from === id || e.to === id) {
        ...
        if (p) p.setAttribute('d', edgePath(e));
      }
    persist();
  }
  function setSpotlight(ids) {
    ...
    for (const [id, g] of nodeEls) {
      g.classList.toggle('is-spotlight', set.has(id));
      g.classList.toggle('is-dim', !set.has(id));
    }
  }
  function clearSpotlight() {
    for (const [, g] of nodeEls) g.classList.remove('is-spotlight', 'is-dim');
  }
  function getScene() {
