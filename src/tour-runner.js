...
export function startTour({ host, engine, steps, autoplay = false }) {
  ...
  function render() {
    ...
    step.className = 'rl-tour-step';
    step.textContent = `Step ${s.order} of ${steps.length}`;
    const title = document.createElement('div');
    title.className = 'rl-tour-title';
    title.textContent = s.title;
    const blurb = document.createElement('p');
    blurb.className = 'rl-tour-blurb';
    blurb.textContent = s.blurb;
    const ctl = document.createElement('div');
    ctl.className = 'rl-tour-ctl';
    const back = document.createElement('button');
    back.textContent = '← Back';
    back.disabled = i === 0;
    back.onclick = prev;
    const fwd = document.createElement('button');
    fwd.textContent = i === steps.length - 1 ? 'Done' : 'Next →';
    fwd.onclick = () => (i === steps.length - 1 ? exit() : next());
    ctl.append(back, fwd);
    card.append(step, title, blurb, ctl);
    if (s.lesson) {
      const l = document.createElement('div');
      l.className = 'rl-tour-lesson';
      l.textContent = s.lesson;
      card.insertBefore(l, ctl);
    }
    if (autoplay && !reduced) {
      clearTimeout(timer);
      timer = setTimeout(() => (i < steps.length - 1 ? next() : exit()), 6000);
    }
  }
  function go(n) {
