...
export function startCoachmark({ steps, copy, onExit }) {
  ...
  spot.className = 'cm-spotlight';
  const card = document.createElement('div');
  card.className = 'cm-card';
  const veeSlot = document.createElement('div');
  veeSlot.className = 'cm-vee';
  ...
  async function render() {
    ...
    if (el) {
      ...
      const p = placeCard(
        ...
        { w: card.offsetWidth || 320, h: card.offsetHeight || 150 },
        vp
      );
