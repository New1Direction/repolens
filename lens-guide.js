// Authored "How to use it" + "Common misconceptions" copy per framework / single-shot
// lens. Static — mirrors explainers.js. Rendered as a collapsible strip above each
// lens result. Keyed by the framework key (Systems/Ideate/Prioritize) or lens key
// (deepdive/sktpg).

export const LENS_GUIDE = {
  // ─ Ideate (creative) ─
  triz: {
    howToUse: 'Reach for it when two goals fight (speed vs richness). Name the contradiction, then use the principles as a menu of escapes, not a verdict.',
    misconceptions: [
  ...
  scamper: {
    howToUse: 'Each letter forces a specific transformation — read them as prompts, not finished answers. Take the 1–2 that spark something and push them further yourself.',
    misconceptions: [
  ...
  lateral: {
    howToUse: 'Use it when straight logic keeps landing on the obvious. The random provocation is bait — judge the leap it triggers, not the provocation itself.',
    misconceptions: [
  ...
  morph: {
    howToUse: 'Use it to escape one-dimensional thinking. Read the axes, then chase the combinations no one would naturally pick.',
    misconceptions: [
  ...
  // ─ Systems ─
  toc: {
    howToUse: 'Use it to stop optimizing things that do not matter. Fix only the named bottleneck; everything upstream of it is wasted effort until it moves.',
    misconceptions: [
  ...
  loops: {
    howToUse: 'Use it to see why the system accelerates or stalls. Trace each cycle back to its start; reinforcing loops compound, balancing loops resist.',
    misconceptions: [
  ...
  pdca: {
    howToUse: "Use it to judge whether the project actually learns. Look for a real Check step — that's the one teams skip.",
    misconceptions: [
  ...
  dmaic: {
    howToUse: 'Use it when the problem is variance/defects, not features. Insist on a measurable baseline before any improvement.',
    misconceptions: [
  ...
  // ─ Prioritize ─
  pareto: {
    howToUse: 'Use it to find the few factors worth your time. Act on the vital few; consciously defer the long tail.',
    misconceptions: [
  ...
  eisenhower: {
    howToUse: 'Sort honestly by urgency vs importance, then act on the quadrant, not the task: Do, Schedule, Delegate/automate, or Drop.',
    misconceptions: [
  ...
  // ─ single-shot lenses ─
  deepdive: {
    howToUse: "Run it when you need to actually understand the code, not just decide on it. Read the atoms first, then how they depend on each other.",
    misconceptions: [
  ...
  sktpg: {
    howToUse: "Use it to judge trajectory, not present state — where this is heading in 6–18 months and what to do before consensus.",
    misconceptions: [
