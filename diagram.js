...
export function lineageSvg(atoms, links) {
  ...
  for (let i = 0; i < ids.length; i++) {
    let changed = false;
    for (const l of valid) if (depth[l.to] < depth[l.from] + 1) { depth[l.to] = depth[l.from] + 1; changed = true; }
    if (!changed) break;
  }
  ...
  for (let d = 0; d <= maxDepth; d++) {
    ...
    col.forEach((id, i) => { pos[id] = { x: PAD + d * (NODE_W + COL_GAP), y: top + i * (NODE_H + ROW_GAP) }; });
  }
...
export function loopSvg(cycle, type) {
  ...
  const labels = pts.map((p, i) => `<g><circle class="dg-dot ${cls}" cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="5"/><text class="dg-loop-text" x="${p.x.toFixed(1)}" y="${(p.y - 11).toFixed(1)}" text-anchor="middle">${esc(truncate(nodes[i], 16))}</text></g>`).join('');
