...
describe('store backup — exportStores', () => {
  ...
  it('gathers rows from all three stores', async () => {
    ...
    await addEdge({
      id: 'e1',
      source: hashRepoId('a/one'),
      target: hashRepoId('b/two'),
...
describe('store backup — importStores', () => {
  ...
  it('replace mode wipes rows not present in the backup', async () => {
    ...
    await importStores(
      { repos: [{ id: hashRepoId('new/one'), payload: { repoId: 'new/one' } }] },
      { mode: 'replace' }
    );
  ...
  it('skips malformed rows that have no id', async () => {
    const written = await importStores({
      repos: [{ payload: { repoId: 'x' } }, null, { id: 5, payload: { repoId: 'ok/one' } }],
    });
...
describe('store backup — importStores merge unions snapshot history (B-1)', () => {
  ...
  it('unions local + incoming snaps by ts (no local loss), deduped, clamped to the cap', async () => {
    ...
    await idbPut('snapshots', {
      ...
      repoId: 'm/one',
      snaps: [
        snap('2026-01-01T00:00:00.000Z', 50),
        snap('2026-01-02T00:00:00.000Z', 55),
        snap('2026-01-03T00:00:00.000Z', 60),
        snap('2026-01-04T00:00:00.000Z', 65),
      ],
    });
      ...
      snapshots: [
        {
          ...
          repoId: 'm/one',
          snaps: [
            snap('2026-01-03T00:00:00.000Z', 99),
            snap('2026-01-05T00:00:00.000Z', 70),
            snap('2026-01-06T00:00:00.000Z', 75),
          ],
        },
      ],
    };
      ...
      snapshots: [
        {
          id,
          repoId: 'm/cap',
          snaps: Array.from({ length: 5 }, (_, i) => snap(day(SNAPSHOT_CAP + i), 100 + i)),
        },
      ],
    };
    await importStores(incoming, { mode: 'merge' });
  ...
  it('merge mode adds a brand-new repo snapshot row untouched', async () => {
    ...
    await importStores(
      { snapshots: [{ id, repoId: 'm/new', snaps: [snap('2026-03-01T00:00:00.000Z', 80)] }] },
      { mode: 'merge' }
    );
  ...
  it('replace mode still overwrites a local snapshot row wholesale', async () => {
    ...
    await idbPut('snapshots', {
      id,
      repoId: 'r/one',
      snaps: [snap('2026-01-01T00:00:00.000Z', 50), snap('2026-01-02T00:00:00.000Z', 55)],
    });
    await importStores(
      { snapshots: [{ id, repoId: 'r/one', snaps: [snap('2026-04-01T00:00:00.000Z', 90)] }] },
      { mode: 'replace' }
    );
