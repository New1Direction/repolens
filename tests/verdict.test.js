...
describe('deriveFit', () => {
  ...
  it('strong: high health, zero warnings', () => {
    expect(deriveFit({ health: { score: 92 }, red_flags: [{ severity: 'ok' }], pros: [1, 2, 3], cons: [1] }))
  ...
  it('care: mixed health, a couple warnings', () => {
    expect(fit({ health: { score: 54 }, red_flags: [{ severity: 'warning' }, { severity: 'warning' }] })).toBe('care');
  });
  it('risky: low health, many warnings', () => {
    expect(fit({ health: { score: 30 }, red_flags: [1, 2, 3, 4].map(() => ({ severity: 'warning' })) })).toBe('risky');
  });
...
describe('verdictCopyText', () => {
  it('builds a plain-text summary with title, fit, bottom line and flags', () => {
    const d = {
      repoId: 'o/r', description: 'A thing.', bottom_line: 'Use it when X.',
      health: { score: 78 }, pros: [1, 2], cons: [1], red_flags: [{ severity: 'warning', title: 'Risk', text: 'Watch out.' }, { severity: 'ok', title: 'Good', text: 'Nice.' }],
    };
