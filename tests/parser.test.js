...
const validResponse = {
  eli5: 'Simple explanation.', technical: 'Technical detail.',
  use_cases: { core_fit: 'a', good_fit: 'b', works_well: 'c', long_term: 'd' },
  skip_if: { overkill: 'a', wrong_tool: 'b', needs_care: 'c', consider: 'd' },
  enables: 'Enables stuff.', pros: ['Pro one', 'Pro two'], cons: ['Con one'],
  alternatives: [{ name: 'Vue', when: 'Gentler learning curve.' }],
  health: { score: 85, commit_activity: 90, issue_response: 70, pr_merge_rate: 80, maintainer_count: 85, summary: 'Healthy.' },
  red_flags: [{ title: 'Flag', text: 'Detail.', severity: 'warning' }],
  start_here: [{ icon: '📖', title: 'Docs', desc: 'Start here.', tag: 'DOCS' }],
  compare_hooks: 'React beats Vue on ecosystem size.', tags: ['javascript', 'ui'], category: 'UI Framework'
};
...
describe('parseClaudeResponse', () => {
  ...
  it('parses analogies and defaults to an empty array', () => {
    ...
    expect(parseClaudeResponse(JSON.stringify(withA)).analogies).toEqual(['like a kitchen', 'like a switchboard']);
  ...
  it('normalizes highlights: defaults empty, drops blanks, clamps severity/tab, caps at 4', () => {
    ...
    const withH = { ...validResponse, highlights: [
    ...
    const many = { ...validResponse, highlights: Array.from({ length: 7 }, (_, i) => ({ text: `h${i}`, severity: 'insight', tab: 'eli5' })) };
    expect(parseClaudeResponse(JSON.stringify(many)).highlights.length).toBe(4);
  });
  it('parses tech_stack and defaults it when missing', () => {
    const withTs = { ...validResponse, tech_stack: { built_with: ['TypeScript'], key_dependencies: [{ name: 'scheduler', purpose: 'scheduling' }] } };
    expect(parseClaudeResponse(JSON.stringify(withTs)).tech_stack.built_with).toEqual(['TypeScript']);
    expect(parseClaudeResponse(JSON.stringify(validResponse)).tech_stack).toEqual({ built_with: [], key_dependencies: [] });
  });
